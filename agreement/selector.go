...
func membership(l LedgerReader, addr basics.Address, r basics.Round, p period, s step) (m committee.Membership, err error) {
	...
	m.MergedStake = basics.ConstantMerger{Retriever: l}.Merge(r, record.VotingStake(), record.Scores)
	m.TotalMergedStake = basics.ConstantMerger{Retriever: l, Total: true}.Merge(r, total, scores)
	return m, nil
}
